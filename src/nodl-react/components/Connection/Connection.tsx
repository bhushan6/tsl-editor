  ...
  <T,>({ output, connection }: ConnectionProps<T>) => {
    ...
    React.useEffect(() => {
      if (outputElement && inputElement) {
        return autorun(() => {
          if (connection) {
            const fromPosition = store.nodePositions.get(
              store.getNodeByPortId(connection.from.id)?.id || ""
            );
            const toPosition = store.nodePositions.get(
              store.getNodeByPortId(connection.to.id)?.id || ""
            );
    ...
    React.useEffect(() => {
      if (output && outputElement) {
        return autorun(() => {
          const outputPosition = store.nodePositions.get(
            store.getNodeByPortId(output.id)?.id || ""
          );
